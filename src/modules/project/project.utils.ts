...
type ProjectWithMilestonesAndClient = Prisma.ProjectGetPayload<{
  include: {
...
export function serializeProjectWithMilestones(
  project: ProjectWithMilestones | ProjectWithMilestonesAndClient,
) {
  return {
    id: project.id,
...
// Serialize Project for Client View
// Includes freelancer info so the client knows who they're working with
export type ProjectForClient = Prisma.ProjectGetPayload<{
  include: {
...
export function serializeProjectForClient(project: ProjectForClient | null) {
  if (!project) return null;
  return {
    id: project.id,
    title: project.title,
    ...
    freelancer: project.freelancer
      ? { name: project.freelancer.name, email: project.freelancer.email }
      : null,
    milestones: project.milestones.map(serializeMilestone),
    invoices: project.invoices.map(serializeInvoice),
    feedbacks: project.feedbacks.map(serializeFeedback),
